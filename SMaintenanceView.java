...
public class SMaintenanceView extends JFrame{
    ...
    public SMaintenanceView(VendingMachine vm){
        super("Maintaining Special Vending Machine");

        this.svm = (SpecialVendingMachine) vm;
    ...
    public String getItemInventory(){
        ...
        for (int i = 0; i < 12; i++){
            index = i+1;
            if(svm.slots[i].getNumItems()==0){
                text += "[" +index+ "] " + itemNamesList[i] + ": " + svm.slots[i].getNumItems()+ "pcs.\n";
            }
            else{
                text += "[" +index+ "] " + itemNamesList[i] + ": " + svm.slots[i].getNumItems()+ "pcs. - P" + svm.slots[i].getPrice() + "\n";
            }
        }
        return text;
    }
